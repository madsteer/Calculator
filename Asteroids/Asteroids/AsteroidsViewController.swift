...
class AsteroidsViewController: UIViewController {
    ...
    private var asteroidBehavoir = AsteroidBehavior()
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        initializeIfNeeded()
        animator.addBehavior(asteroidBehavoir)
        asteroidBehavoir.pushAllAsteroids()
    }
    
    override func viewWillDisappear(_ animated: Bool) {
        super.viewWillDisappear(animated)
        animator.removeBehavior(asteroidBehavoir)
    }
    ...
    private func initializeIfNeeded() {
        if asteroidField == nil {
            ...
            asteroidField.asteroidBehavior = asteroidBehavoir
        }
    }
    ...
    private func repositionShip() {
        if let asteroidField = asteroidField {
            ship.center = asteroidField.center
            asteroidBehavoir.setBoundary(ship.shieldBoundary(in: asteroidField), named: Constants.shipBoundaryName) {
                [weak self] in
                if let ship = self?.ship, !ship.shieldIsActive {
    ...
    private func burn() {
        ...
        asteroidBehavoir.acceleration.angle = ship.direction - CGFloat.pi
        asteroidBehavoir.acceleration.magnitude = Constants.burnAcceleration
    }
    
    private func endBurn() {
        ship.enginesAreFiring = false
        asteroidBehavoir.acceleration.magnitude = 0
    }
